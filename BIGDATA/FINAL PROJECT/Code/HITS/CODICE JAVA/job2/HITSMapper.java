...
public class HITSMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text>{

    public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
        int userTabIndex = value.find("*");
        int authTabIndex = value.find("-", userTabIndex+1);
        int hubTabIndex  = value.find("_", authTabIndex+1);
        ...
        String[] outgoingLinks = linksInOut[0].replaceAll("\t","").split(",");
        ...
        String[] incomingLinks = linksInOut[1].replaceAll("\t","").split(",");
