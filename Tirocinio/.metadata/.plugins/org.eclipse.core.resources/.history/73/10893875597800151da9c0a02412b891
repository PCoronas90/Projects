...
public class SignalProcessor {
	...
	public static ArrayList<ArrayList<Double>> computeMediumEnergy(Signal s, int length, double energy, int attempts, int inf,
		...
		for (double snr = inf; snr < sup; snr++) {
			...
			for (int j = 0; j < attempts; j++) {
				Noise noise = new Noise(snr,s.getSignalLenght()-1, energy);
				
				double avg = 0;
				int samples = (length/block);
				int startIndex=0;
				for(int i=0;i<s.getSignalLenght();i++){
					if(i%samples==0 & i!=0|i==s.getSignalLenght()-1){
						Signal signal = new Signal(samples);
						ArrayList<Double> samplesRe=MathFunctions.SumVector(splitNoise(noise,startIndex,i).getSamplesRe(), splitSignal(s,startIndex,i).getSamplesRe());
						ArrayList<Double> samplesIm=MathFunctions.SumVector(splitNoise(noise,startIndex,i).getSamplesIm(), splitSignal(s,startIndex,i).getSamplesIm());
						signal.setSamplesRe(samplesRe);
						signal.setSamplesIm(samplesIm);
						signal.setSignalLenght(samplesRe.size());
						startIndex=i;
	...
	public static Signal splitSignal(Signal signal,int start,int end){
	...
	public static Noise splitNoise(Noise noise,int start,int end){
		ArrayList<Double> samplesRea=new ArrayList<Double>();
		ArrayList<Double> samplesImm=new ArrayList<Double>();
		for(int i=start;i<end-1;i++){
			samplesRea.add(noise.getSamplesRe().get(i));
			samplesImm.add(noise.getSamplesIm().get(i));
			
		}
		noise.setSamplesRe(samplesRea);
		noise.setSamplesIm(samplesImm);
		return noise;
	}
