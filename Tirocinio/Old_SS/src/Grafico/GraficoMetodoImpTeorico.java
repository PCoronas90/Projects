...
public class GraficoMetodoImpTeorico  extends Grafico {
...
//Genera il grafico del metodo implementato tramite calcolo della detection teorica
	public  GraficoMetodoImpTeorico(String titolo,ArrayList<Pr> prSegnale, ArrayList<Pr>  prRumore) throws Exception {
		...
		setContentPane(chartPanel);
			...
			try {
				...
				ChartUtilities.saveChartAsPNG(new File("C:\\Grafico2.png"), chart, 650, 450);
	...
	protected XYDataset createDataset() throws Exception {
		...
		final XYSeries serie1 = generaSerie("Detection Metodo implementato (Teorica)");
		...
		XYSeries serie = new XYSeries(label);
			...
			for(int i=0;i<this.PRSegnale.size();i++){
				serie.add(this.PRSegnale.get(i).getMomenti().getSnr(),
						CalcoloDelleDetection.calcoloDetectionTeorica(CalcoloSoglie.calcoloSoglia(0.01,this.PRRumore.get(i)),this.PRSegnale.get(i)));
		}
