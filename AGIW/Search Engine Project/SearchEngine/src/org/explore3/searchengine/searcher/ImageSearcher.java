...
public class ImageSearcher {
	...
	public ImageSearcher() {
		...
		this.pathOfIndex = "C:/Users/Pietro/Desktop/Workspace/Explore3SearchEngine/Imageindex";
	}
	...
	public Search search(String queryString) throws ParseException, IOException, InvalidTokenOffsetsException {
		...
		QueryParser queryParser = new QueryParser(Version.LUCENE_47, "image", analyzer);
		...
		for (int i = 0; i < hits.totalHits; i++) {
			...
			TokenStream tokenStream = TokenSources.getAnyTokenStream(indexSearcher.getIndexReader(), id, "image",
					analyzer);
			TextFragment[] frag = highlighter.getBestTextFragments(tokenStream, image, false, 4);

			Result r = new Result();
			r.setTitle(doc.get("title"));
						
			String path = doc.get("image");			
			r.setPath(path);
